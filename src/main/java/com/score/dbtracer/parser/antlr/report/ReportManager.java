package com.score.dbtracer.parser.antlr.report;


import java.text.SimpleDateFormat;

import com.score.dbtracer.parser.antlr.resdef.Entity;
import com.score.dbtracer.parser.antlr.resdef.Field;
...
public class ReportManager {
        ...
        if (!hasValidationWarning) {
            ...
            for (final ReportLine reportLine : report.getReportLines()) {
                if ((reportLine.getReportLineStatus() == ReportLineStatus.UNKNOWN_SQL_TYPE) ||
                    ...
                    if (reportLine.getReportLineStatus() == ReportLineStatus.UNKNOWN_SQL_TYPE) {
                        out.append("The SQL type \"" + reportLine.getMessage() +
                                "\" is not supported and replaced by the “String” type");
                    }
                }
            }
        }
    ...
    public String getTrace6(final Report report) {
        ...
        if (reportStatus == ReportStatus.SUCCESS) {
            out.append("\n - Location : SQL schema successfully imported : " + report.getNbCreatedEntity() +
                    " entities created");
        ...
        for (final Table table : report.getDatabase().getTables()) {
            ...
            if (entity != null) {
                for (final Column column : table.getColumnByNames().values()) {
                    ...
                    out.append("\n    - NotNull : ")
                       .append((column.getIsNotNull() == null) ? "<<empty>>" : column.getIsNotNull())
                       .append(" -> Nullable : ")
                       .append(field.getNullable());
                }
                // primary key
                out.append("\n   - Primary key : [");
